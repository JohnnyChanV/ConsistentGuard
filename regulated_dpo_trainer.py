...
from R1_components.a_dpo_trainer import DPOTrainer
...
def set_seed(seed: int):
    """设置Python、NumPy和PyTorch的随机种子，以确保实验可复现"""
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)  # 确保所有GPU的随机性一致
    torch.backends.cudnn.deterministic = True  # 确保 CNN 结果可复现
    torch.backends.cudnn.benchmark = False  # 可能会降低训练速度，但能保证结果一致
...
def get_args():
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/GRPO-Model-3B-RAO")
    parser.add_argument("--run_name", type=str, default="GRPO-Model-3B-RAO")
