...
class DataCollatorForPreference(DataCollatorMixin):
    ...
    def torch_call(self, examples: list[Union[list[int], Any, dict[str, Any]]]) -> dict[str, Any]:
        ...
        anchor_prompt_input_ids = [torch.tensor(example["anchor_prompt_input_ids"]) for example in examples]
        anchor_prompt_attention_mask = [torch.ones_like(input_ids) for input_ids in anchor_prompt_input_ids]
        anchor_completion_input_ids = [torch.tensor(example["anchor_completion_input_ids"]) for example in examples]
        anchor_completion_attention_mask = [torch.ones_like(input_ids) for input_ids in anchor_completion_input_ids]
        ...
        output["anchor_prompt_input_ids"] = pad(anchor_prompt_input_ids, padding_value=self.pad_token_id, padding_side="left")
        output["anchor_prompt_attention_mask"] = pad(anchor_prompt_attention_mask, padding_value=0, padding_side="left")
        output["anchor_completion_input_ids"] = pad(anchor_completion_input_ids, padding_value=self.pad_token_id)
        output["anchor_completion_attention_mask"] = pad(anchor_completion_attention_mask, padding_value=0)
...
class DPOTrainer(Trainer):
    ...
    def _prepare_dataset(
        ...
        with PartialState().local_main_process_first():
            ...
            dataset = dataset.map(
                ...
                remove_columns=["prompt", "chosen", "rejected","anchor_prompt","anchor_completion"],
                fn_kwargs={
                    "processing_class": processing_class,
    ...
    def tokenize_row(features, processing_class, max_prompt_length, max_completion_length, add_special_tokens):
        ...
        anchor_prompt_input_ids = tokenizer(features["anchor_prompt"], add_special_tokens=False)["input_ids"]
        anchor_completion_input_ids = tokenizer(features["anchor_completion"], add_special_tokens=False)["input_ids"]
        ...
        if add_special_tokens:
            if tokenizer.bos_token_id is not None:
                ...
                anchor_prompt_input_ids = [tokenizer.bos_token_id] + anchor_prompt_input_ids
            if tokenizer.eos_token_id is not None:
                prompt_input_ids = prompt_input_ids + [tokenizer.eos_token_id]
                anchor_prompt_input_ids = anchor_prompt_input_ids + [tokenizer.eos_token_id]

        chosen_input_ids = chosen_input_ids + [tokenizer.eos_token_id]
        rejected_input_ids = rejected_input_ids + [tokenizer.eos_token_id]
        anchor_completion_input_ids = anchor_completion_input_ids + [tokenizer.eos_token_id]

        # Truncate prompt and completion sequences
        if max_prompt_length is not None:
            prompt_input_ids = prompt_input_ids[-max_prompt_length:]
            anchor_prompt_input_ids = anchor_prompt_input_ids[-max_prompt_length:]
        if max_completion_length is not None:
            chosen_input_ids = chosen_input_ids[:max_completion_length]
            rejected_input_ids = rejected_input_ids[:max_completion_length]
            anchor_completion_input_ids = anchor_completion_input_ids[:max_completion_length]
        ...
        return {
            ...
            "anchor_prompt_input_ids": anchor_prompt_input_ids,
            "anchor_completion_input_ids": anchor_completion_input_ids
        }
    ...
    def _set_signature_columns_if_needed(self):
        ...
        if self._signature_columns is None:
            self._signature_columns = [
                ...
                'anchor_prompt_input_ids',
                'anchor_completion_input_ids',
    ...
    def compute_ref_log_probs(self, batch: dict[str, torch.LongTensor], get_anchor=False) -> dict:
        ...
        if not get_anchor:
            ...
            return ref_model_output["chosen_logps"], ref_model_output["rejected_logps"], ref_model_output['anchor_logits']
    ...
    def concatenated_inputs(
        ...
        max_prompt_length = max(batch["prompt_input_ids"].shape[1], batch["anchor_prompt_input_ids"].shape[1])
        output["prompt_input_ids"] = torch.cat(
            (
                pad_to_length(batch["prompt_input_ids"], max_prompt_length, pad_value=padding_value),
                pad_to_length(batch["prompt_input_ids"], max_prompt_length, pad_value=padding_value),
                pad_to_length(batch['anchor_prompt_input_ids'], max_prompt_length, pad_value=padding_value)
            ),
        )
        output["prompt_attention_mask"] = torch.cat(
            (
                pad_to_length(batch["prompt_attention_mask"], max_prompt_length, pad_value=0),
                pad_to_length(batch["prompt_attention_mask"], max_prompt_length, pad_value=0),
                pad_to_length(batch["anchor_prompt_attention_mask"], max_prompt_length, pad_value=0),
            ),
        )

        # output["prompt_input_ids"] = torch.cat([batch["prompt_input_ids"], batch["prompt_input_ids"], batch['anchor_prompt_input_ids']], dim=0)
        # output["prompt_attention_mask"] = torch.cat(
        #     [batch["prompt_attention_mask"], batch["prompt_attention_mask"], batch['anchor_prompt_attention_mask']], dim=0
        # )
        ...
        max_completion_length = max(batch["chosen_input_ids"].shape[1], batch["rejected_input_ids"].shape[1], batch["anchor_completion_input_ids"].shape[1])
        output["completion_input_ids"] = torch.cat(
            (
                pad_to_length(batch["chosen_input_ids"], max_completion_length, pad_value=padding_value),
                pad_to_length(batch["rejected_input_ids"], max_completion_length, pad_value=padding_value),
                pad_to_length(batch['anchor_completion_input_ids'], max_completion_length, pad_value=padding_value)
            ),
        )
        output["completion_attention_mask"] = torch.cat(
            (
                ...
                pad_to_length(batch["anchor_completion_attention_mask"], max_completion_length, pad_value=0),
            ),
        )
    ...
    def concatenated_forward(self, model: nn.Module, batch: dict[str, Union[list, torch.LongTensor]]):
        ...
        if self.use_weighting:
            with torch.no_grad():
                ...
                rejected_weights = all_weights[num_examples:num_examples*2]
        ...
        output["rejected_logps"] = all_logps[num_examples:num_examples*2]
        output["anchor_logps"] = all_logps[num_examples * 2:num_examples * 3]
        ...
        if self.padding_free:
            ...
            mean_rejected_logits = logits[num_examples:num_examples*2][loss_mask[num_examples:num_examples*2]].mean()

        output["mean_chosen_logits"] = mean_chosen_logits
        output["mean_rejected_logits"] = mean_rejected_logits
        # print(loss_mask[num_examples*2:num_examples*3][0])

        output["anchor_logits"] = logits[num_examples*2:num_examples*3][attention_mask[num_examples*2:num_examples*3].bool()]
        # print(logits[num_examples*2:num_examples*3])
        # [attention_mask[num_examples*2:num_examples*3]]
        # print(output["anchor_logits"].mean())
    ...
    @staticmethod
    def kl_divergence(A, B, epsilon=1e-9, tol=1e-9):
        """
        计算 KL 散度，并在 A ≈ B 时返回 0，同时处理 NaN 值

        参数：
        A: torch.Tensor - 预测分布 (batch, length, vocab)
        B: torch.Tensor - 真实分布 (batch, length, vocab)
        epsilon: float - 避免 log(0) 的极小值
        tol: float - 判断 A ≈ B 的误差阈值

        返回：
        KL 散度: torch.Tensor (batch,)
        """
        # 确保 A 和 B 是有效概率分布
        A = A / A.sum(dim=-1, keepdim=True)  # 归一化
        B = B / B.sum(dim=-1, keepdim=True)  # 归一化
        #
        # 避免 log(0) 计算问题
        A = A.clamp(min=epsilon)
        B = B.clamp(min=epsilon)

        # 计算 KL 散度
        log_A = A.log()
        kl = F.kl_div(log_A, B, reduction='none').sum(dim=-1).mean() # shape (batch,)

        # 处理 NaN 情况
        kl = torch.nan_to_num(kl, nan=0.0)

        # 处理 A ≈ B 的情况：如果 KL 散度小于 tol，设置为 0
        kl = torch.where(kl < tol, torch.tensor(0.0, device=kl.device), kl)
    ...
    def get_batch_loss_metrics(
        ...
        # else:
        ref_chosen_logps, ref_rejected_logps, ref_anchor_logits = self.compute_ref_log_probs(batch,get_anchor=True)
        ...
        kl_loss = self.kl_divergence(model_output['anchor_logits'],ref_anchor_logits).mean()
        ...
        metrics[f"{prefix}loss-item/kl"] = (
            self.accelerator.gather_for_metrics(kl_loss).detach().mean().item()
        )
        ...
        return losses.mean() + kl_loss, metrics
